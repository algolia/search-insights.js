...
export function click(params: ClickReport) {
  if(!this._hasCredentials) {
    ...
    throw new Error('No params were sent to click function, please provide an objectID and position to be reported')

  } else if(!params.objectID) {
    throw new Error('required objectID parameter was not sent, click event can not be properly attributed')

  } else if(!params.position) {
    throw new Error('required position parameter was not sent, click event position can not be properly sent without')
  }
