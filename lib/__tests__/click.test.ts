...
describe('Click method', () => {
  ...
  test('Should throw if objectID and position are not sent', () => {
    ...
    expect(() => {
      AlgoliaInsights.click()
    }).toThrowError('No params were sent to click function, please provide an objectID and position to be reported')
  })
  ...
  test('Should throw if objectID is not sent', () => {
    ...
    }).toThrowError('required position parameter was not sent, click event position can not be properly sent without')
  })
  ...
  test('Should call sendEvent with proper params', () => {
    const clickParams = {position: 1, objectID: '2', queryID: 5}

    AlgoliaInsights.init(credentials);
    AlgoliaInsights.sendEvent = jest.fn()
    AlgoliaInsights.click(clickParams);

    expect(AlgoliaInsights.sendEvent).toHaveBeenCalledWith('click', clickParams});
  }
});
