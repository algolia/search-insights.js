...
export function initSearch(initParams: initSearchParams) {
  if(!this._hasCredentials) {
    ...
    throw new Error('initSearch function requires an argument with getQueryID and hitsContainer arguments')

  } else if(!initParams.getQueryID || typeof initParams.getQueryID !== 'function') {
    throw new Error('getQueryID must be a function that returns the queryID of the last search operation')
  }
