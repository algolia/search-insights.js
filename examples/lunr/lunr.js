...
var documents = [
  ...
  },
  {
    ...
    "name": "Lodash",
    "text": "A modern JavaScript utility search delivering modularity, performance & extras.",
    "objectID": "test_3"
}]

var hitTemplate = (hit, i) => `
<article id="hit${hit.objectID}" class="hit" data-algolia-objectid="${hit.objectID}">
  <div class="product-picture-wrapper">
  </div>
  <div class="product-desc-wrapper">
    <div class="product-name">${hit.name}</div>
  </div>
  <button objectID="${hit.objectID}" position="${i + 1}" class="button-click" style="background: blue;padding: 10px 12px; color: white;">click</button>
  <a data-objectID="${hit.objectID}" position="${i + 1}" href="http://localhost:8080/product.html" class="button-click" style="background: blue;padding: 10px 12px; color: white;">VIEW ITEM</a>
  <button objectID="${hit.objectID}" class="button-convert" style="background: blue;padding: 10px 12px; color: white;">ADD TO CART</button>
</article>`;
